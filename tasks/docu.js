...
module.exports = function( grunt ) {
     ...
     */
    grunt.registerTask('docu', 'Compile Documentation to HTML', function() {
        var done = this.async();
        ...
        function generateDocs() {
            ...
            var base = 'content/documentation/';
            ...
            function generateGuides() {
                ...
                sidebars[2] = getSidebarSection('### Tools');
                sidebars[3] = getSidebarSection('### API');

                names.forEach(function( name ) {

                    var title = name.replace(/-/g, ' ').replace('.md', ''), segment = name.replace(/ /g, '-').replace('.md', '').toLowerCase(), src = base + name, dest = 'build/docu/' + name.replace('.md', '').toLowerCase() + '/index.html';

                    grunt.file.copy(src, dest, {
                        process: function( src ) {
                            try {
                                var file = 'src/tmpl/docu.jade', templateData = {
                                        page: 'docu',
                                        rootSidebar: true,
                                        pageSegment: segment,
                                        title: title,
            ...
            function generateAPI() {
                ...
                // API Docs
                var sidebars = [];
                var names = grunt.file.expand({cwd: base}, ['*.md']);
            ...
            function getSidebarSection( section, iconClass ) {
                ...
                for( l in lines ) {
                    ...
                    if( rMode && line.length > 0 ) {
                        var item = line.replace(/#/g, '').replace(']]', '').replace('* [[', ''), url = 'docu/' + item;
